...
Проект «Визуализация открытых данных Росстата» - собираем, конвертируем и визуализируем статистику, публикуемую [Росстатом](https://rosstat.gov.ru).  
...
2. Сохраните его в соответствующей тематической папке, например `data/innovations/...`.
...
5. Поместите SQL‑запрос в файл `*.sql` и выполните его в pgAdmin. 
6. Синхронизируйте данные в Metabase (нужен админ доступ, пишите в tg @ailabintsev) 
7. Создайте новый дашборд в Metabase, используя таблицы, которые вы создали в postgres. 
8. Если все получилось, можно делать коммит. В коммите должны быть файлы:
    - **`*.xls`** – оригинальный файл, скачанный с сайта Росстата.
    - **`*.txt`** – промпты (инструкции) для генерации SQL‑запроса из таблицы (несколько, по количеству вкладок).
    - **`*.sql`** – готовые SQL‑запросы для создания и заполнения таблиц в PostgreSQL.

9. Сделайте push в свой репозиторий. 
10. Сделайте pull request в основной репозиторий. Прикрепите ссылку на дашборд в комментарии к pull request.
